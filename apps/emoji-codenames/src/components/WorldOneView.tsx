...
export function WorldOneView({ level, onCorrect }: WorldOneViewProps) {
    ...
    return (
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.5rem', opacity: 0.7 }}>Which picture matches the code?</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: '500px' }}>
                {options.map(option => (
                    <Button
                        key={option}
                        variant={wrong === option ? 'secondary' : 'primary'}
                        onClick={() => handleGuess(option)}
                        style={{ height: '120px', fontSize: '3rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}
                    >

...
export function WorldThreeView({ level, onCorrect }: WorldThreeViewProps) {
    ...
    const family = level.family ? WORD_FAMILIES[level.family as keyof typeof WORD_FAMILIES] : [];
    // Pick 3 random from family including target, or use all if small
    const options = family.length > 0 ? family.slice(0, 3) : [level.targetWord, 'BAD', 'MAD'];
    ...
    return (
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.5rem', opacity: 0.7 }}>Family: -{level.family}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', width: '100%', maxWidth: '600px' }}>
                {options.map(option => (
                    <Button
                        key={option}
                        size="large"
                        variant={option === level.targetWord ? 'primary' : 'secondary'}

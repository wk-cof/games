...
const LETTER_TO_EMOJI: Record<string, string> = Object.entries(EMOJI_TO_LETTER).reduce(
...
export function WorldFourView({ level, onCorrect }: WorldFourViewProps) {
    const [slots, setSlots] = useState<string[]>(new Array(level.targetWord.length).fill(''));
    ...
    return (
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.5rem', opacity: 0.7 }}>Spell the word: <strong>{level.targetWord}</strong></h2>

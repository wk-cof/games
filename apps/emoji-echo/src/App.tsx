...
export default function App() {
    const { gameState, score, activeButton, startGame, handleInput, sequence } = useGameLogic();
    ...
    return (
        <Shell
            ...
            actions={<Button variant="ghost" onClick={() => setShowSettings(true)}>⚙️</Button>}
        >
            <div css={containerStyles}>
                ...
                >
                    ...
                    `}>
                            <div>
                                <div css={css`font-weight: bold;`}>Practice Mode</div>
                                <div css={css`font-size: 0.9rem; color: #666;`}>Show pattern sequence</div>
                            </div>
                            <label css={css`
                        position: relative;
                      ...
                      `}>
                                <input
                                    ...
                                    css={css`opacity: 0; width: 0; height: 0;`}
                                />
                                <span css={css`
                          position: absolute;
                          cursor: pointer;
                          ...
                          &:before {
                            ...
                            transition: .3s;
                            border-radius: 50%;
                            transform: ${isPracticeMode ? 'translateX(22px)' : 'none'};
                          }
                        `}></span>
                            </label>
                        </div>
                ...
                {isPracticeMode && sequence.length > 0 && (
                    <div css={practiceSequenceStyles}>
                        {sequence.map((idx, i) => (
                            <div key={i} title="Practice hint">{BUTTON_CONFIG[idx].emoji}</div>
                        ))}
                    </div>
                )}

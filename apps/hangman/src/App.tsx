...
export default function App() {
    ...
    const handleGuess = (char: string) => {
        ...
        if (!target.word.includes(char)) {
            ...
            // Check win
            const isWin = target.word.split('').every(c => guessed.has(c) || c === char);
            if (isWin) {
                setStatus('won');
                setScore(s => s + 10);
                confetti({
    ...
    const actions = (
        <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <Button variant="ghost" onClick={pickNewWord}>Skip Word</Button>
        </div>
    );
        ...
        <Shell title="Word Whiz" hud={<HUD items={hudItems} />} actions={actions}>
            <div css={gameContainerStyles}>
                ...
                {status === 'playing' ? (
                    ...
                    <div style={{ textAlign: 'center' }}>
                        <h2 style={{ fontSize: '2rem', marginBottom: '1rem', color: status === 'won' ? '#10b981' : '#ef4444' }}>
                            {status === 'won' ? 'You Saved the Snowman! 🎉' : 'Oh no! The Snowman Melted! ☀️'}
                        </h2>
                        <Button onClick={pickNewWord} style={{ fontSize: '1.2rem', padding: '1rem 2rem' }}>
                            Play Again
                        </Button>
                    </div>
                )}

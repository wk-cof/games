...
export default function App() {
    ...
    const hudItems = [
        ...
        ...(mode === 'challenge' && targetEmoji ? [{ label: 'Find', value: targetEmoji }] : [])
    ];

    const actions = isPlaying ? (
        <Button variant="ghost" onClick={() => setIsPlaying(false)}>Stop</Button>
    ) : null;
            ...
            <div css={gameContainerStyles}>
                {!isPlaying ? (
                    <div style={{
                        ...
                        <h1 style={{ fontSize: '3rem', marginBottom: '1rem' }}>Bubble Pop! 🫧</h1>
                        <Button onClick={() => startGame('zen')} style={{ width: '200px' }}>
                            Zen Mode 🧘
                        </Button>
                        <Button onClick={() => startGame('challenge')} variant="ghost" style={{ width: '200px' }}>
                            Challenge Mode 🏆
                        </Button>
                    </div>
                ) : (

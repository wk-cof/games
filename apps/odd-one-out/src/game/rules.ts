...
const attributeNonMatches = (
  attribute: EmojiAttribute,
  themes: ThemeId[],
): EmojiPick[] =>
  themes
...
const allAttributes = Array.from(
  new Set(Object.values(ATTRIBUTE_MAP).flatMap((attrs) => attrs)),
) as EmojiAttribute[];

const attributeCandidates = (themes: ThemeId[]): EmojiAttribute[] =>
  allAttributes.filter(
    (attr) =>
      attributeMatches(attr).length >= 3 &&
      attributeNonMatches(attr, themes).length > 0,
  );

const patternEligible = {
  category: (themes: ThemeId[]): boolean =>
    themes.filter((theme) => getThemeEmojis(theme).length >= 3).length >= 2,
  attribute: (themes: ThemeId[]): boolean =>
    attributeCandidates(themes).length > 0,
...
const getOrientationFriendly = (theme: ThemeId): EmojiPick[] =>
  getThemeEmojis(theme).filter((pick) =>
    ORIENTATION_FRIENDLY_EMOJIS.has(pick.emoji),
  );
  ...
  const tiles = shuffle(
    [
      ...matchPicks.map((pick, index) =>
        toTile(pick, input.round, index, false),
      ),

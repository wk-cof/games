...
export function GameScreen() {
  const { state, settings, announcement, bestScore, restart, selectTile } =
    useGame();
  ...
  const hudItems = useMemo<HUDItem[]>(() => {
    const livesValue =
      settings.mode === "practice" ? "∞" : Math.max(0, state.lives);
    return [
      ...
      { label: "score", value: state.score },
      {
        label: "best",
        value: bestScore,
        tone: state.score >= bestScore && bestScore > 0 ? "success" : "neutral",
        active: state.score >= bestScore && bestScore > 0,
      },
      { label: "streak", value: state.streak },
      { label: "lives", value: livesValue },
      {
        label: "time",
        value: Number.isFinite(state.timeLeftMs)
          ? `${(state.timeLeftMs / 1000).toFixed(state.timeLeftMs >= 1000 ? 1 : 2)}s`
          : "∞",
      },
    ];
  ...
  return (
    <>
      ...
      >
        ...
        <p css={ruleStyles}>
          {state.rule.description || "Pick the odd one out!"}
        </p>
      </Shell>
